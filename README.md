...
![alt text](./cka圖庫/k8s-icon.png)

Kubernetes 是目前最流行的 Container Orchestration 系統，不過學習曲線在初期相當陡峭，而這個系列將會帶著初學者從認識 Kubernetes 開始，最終培養出足夠通過 CKA 的基礎能力。 

文章內容會假設讀者從來沒有碰過 K8s，內容都是非常基礎的概念與操作，開頭將會從 K8s 的基礎概念開始介紹，並搭配實作來介紹不同的操作應用，最後在附錄分享 [CKA 的考試技巧與心得](./附錄/CKA%20攻略：我是如何滿分一次通過的？.md)。

> [為何使用 CKA 入門 Kubernetes ?](#用-cka-入門-kubernetes)
...
今天的內容會先介紹「雲原生」的概念並銜接到 Kubernetes 的起源與發展，最後簡單介紹章節規劃。
...
> 假如你今天需要遷移一台虛擬機到你家裏的桌機，只要先在筆電上匯出虛擬機快照，回到家用桌機的 VirtualBox 匯入即可。


不過虛擬機畢竟還是模擬了「一整台電腦」，每台虛擬機都擁有獨立的作業系統，帶對於 APP 「本身」來說，作業系統中的某些功能根本用不到，所以虛擬機還是顯得有些臃腫。所以後來就出現了**容器**。

容器在具備「**高隔離性**」與「**環境轉移方便**」的同時，遠比虛擬機來的更加**輕量**。
...
![it-work-on-my-machine](cka圖庫\mine-machine.jpg)
...
* **隔離不同應用**：假設你又多寫了一個 Java 應用，那組員的桌機和筆電就需要額外安裝 Java 的執行環境與套件。不過在容器的時代，每台電腦都安裝 Docker，無論是什麼語言寫的應用程式，只要個別包成容器，就能透過 Docker 跑起來，且不會互相影響。
...
看完上述介紹後，如果對於容器概念還是不太熟悉的讀者可以參考[這部](https://www.youtube.com/watch?v=ZKdlglAxr7g)影片，看完後相信會更加了解「容器」這項技術的概念與優點。
...
CKA 考試的及格分數為 66 分，筆者在今年八月(2024)順利通過了考試：
